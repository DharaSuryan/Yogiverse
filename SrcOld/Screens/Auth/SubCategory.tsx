...
const SubCategoryScreen: React.FC<NativeStackScreenProps<AuthStackParamList, 'SubCategory'>> = ({
  ...
  const handleSignUp = async () => {
    ...
    try {
      ...
      if (signupData.first_name) formData.append('first_name', signupData.first_name);
      if (signupData.last_name) formData.append('last_name', signupData.last_name);
      if (signupData.email) formData.append('email', signupData.email);
      if (signupData.phone_no) formData.append('phone_no', signupData.phone_no);
      ...
      if (signupData.country_id) formData.append('country', signupData.country_id.toString());
      if (signupData.state_id) formData.append('state', signupData.state_id.toString());
      if (signupData.city_id) formData.append('city', signupData.city_id.toString());
      formData.append('role', 'vendor'); // Always append role
      if (signupData.bio) formData.append('bio', signupData.bio);

      if (signupData.role === 'vendor') {
        if (signupData.business_name) formData.append('business_name', signupData.business_name);
        // The server expects the arrays as a JSON string
        if (signupData.main_categories && signupData.main_categories.length > 0) {
      ...
      const response = await registerUser(formData, 'vendor');
