...
const STORY_DURATION = 6000; // 5 seconds per story
...
const StoryViewerScreen: React.FC<StoryViewerScreenProps> = ({ route, navigation }) => {
  ...
  const deleteStory = async () => {
    console.log("delete story called:", currentStory.id);
    ...
    try {
      ...
      let res = await axios.delete(`https://pashuahar.com/stories/${currentStory.id.toString()}/`, {
        headers,
      });
      console.log(" story response:", res);
      ...
      Alert.alert('Deleted', 'Stories deleted successfully.');
      // showSuccess("Story deleted successfully.");

    } catch (err) {
      console.log("error in delete draft:", err);
  ...
  const confirmDeleteStory = () => {
    Alert.alert(
      ...
      [
        {
          text: "No",
          style: "cancel", // closes dialog
        },
        {
          text: "Yes",
          onPress: () => deleteStory(), // call your function only on Yes
          style: "destructive",
        },
      ],
      { cancelable: true }
  ...
  return (
    <View style={styles.container}>
      ...
      {isPersonal && <TouchableOpacity style={[styles.closeButton, { right: 95, zIndex: 20 }]} onPress={() => {
