...
const UserProfileScreen = ({ navigation }: any) => {
  ...
  useFocusEffect(
    React.useCallback(() => {
      loadAllData(); // Use loadAllData for initial load
      return () => {
  ...
  // Load all data initially (all pages) - for posts and reels
  const loadAllData = async () => {
    ...
    try {
      ...
      // Handle search vs own profile differently
      if (isFromSearch && userId) {
        // For search profiles, get data from search API
        ...
        // Process posts and reels from search response
        ...
        // For own profile, use pagination to load all data
        // First, get the first page to determine total pages
        const firstPageResponse = await getUserPosts(1, undefined);
        ...
        for (let page = 2; page <= totalPages; page++) {
          try {
            ...
            const pageResponse = await getUserPosts(page, undefined);
      ...
      try {
        const storiesRes = await axios.get(`https://pashuahar.com/stories/`, {
          params: { user_id: userId || profile.id },
          headers,
        });
        const storiesData = storiesRes?.data?.data || [];
        setProfileStories(storiesData);
  ...
  const filteredPosts =
    activeTab === 'tagged'
      ? taggedPosts
      : posts.filter(post => {
        if (activeTab === 'posts') return true; // Show both posts and reels in posts tab
  ...
  const renderPostItem = ({ item, index }: { item: any; index: number }) => {
    ...
    return (
      <TouchableOpacity
        ...
        activeOpacity={0.9}>
        <View style={{ position: 'relative', width: '100%', height: '100%' }} pointerEvents="none">
          {!isReel ? (
            <Image
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <Modal
        ...
        {isCommentModalVisible && selectedCommentPost && (
          <View
            style={{
              position: 'absolute', top: 0, left: 0, right: 0, bottom: 0,
              backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'flex-end', zIndex: 210
            }}
          >
      ...
      {/* Likes Modal */}
      <Modal
        visible={showLikesModal && !fullscreenVisible}
        transparent
        animationType="fade"
