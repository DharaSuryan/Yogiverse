...
export const UserProfileScreen = async () => {
  ...
  const renderContentItem = ({ item, index }: any) => {
    ...
    const imageUri = item.compressedUri || item.uri || item.media?.[0]?.media_file || item.media?.[0]?.url;
...
const FullScreenReelItem = React.memo(({
  item, index, profile, navigation, expandedCaptions, toggleCaption, onViewableItemsChanged
  ...
  React.useEffect(() => {
    ...
    if (isReel && item.video_file) {
      (async () => {
        try {
          const result = await VideoCompressor.compress(item.video_file, { compressionMethod: 'auto' });
  ...
  return (
    <View style={styles.fullItemContainer}>
      ...
      <View style={styles.headerOverlay}>
        <Image
          source={{ uri: item.profile?.profile_picture || profile?.profile?.profile_picture || 'https://picsum.photos/100' }}
          style={styles.userAvatar}
        />
        <Text style={styles.usernameOverlay}>@{item.profile?.username || profile?.profile?.username}</Text>
      </View>
      ...
      <View style={{ width: '100%', height: height * 0.6 }}>
        <FlatList
          ...
          renderItem={({ item: mediaItem }) => {
            const isVideo = isReel || (mediaItem.media_file?.endsWith('.mp4'));
            const uri = isVideo
            ...
            return isVideo ? (
              <View style={{ width, height: height * 0.6, position: 'relative' }}>
                ...
                >
                  <Icon
                    name={isVideoPaused ? 'play-circle-outline' : 'pause-circle-outline'}
                    size={48}
                    color="#fff"
                  />
                </TouchableOpacity>
          ...
          onViewableItemsChanged={({ viewableItems }) => {
            ...
            if (onViewableItemsChanged) onViewableItemsChanged({ viewableItems });
          }}
      ...
      <View style={styles.scrollableContent}>
        ...
        <Post
          id={item.id?.toString()}
          username={item.profile?.username || profile?.profile?.username || ''}
          media={mediaList}
          caption={item.caption || ''}
          likes={item.like_count || 0}
          userAvatar={item.profile?.profile_picture || profile?.profile?.profile_picture}
        ...
        <ScrollView>
            ...
            {userId !== profile?.profile?.id && (
              <View style={{ alignItems: 'center', marginTop: 8 }}>
                <TouchableOpacity
                  style={[
                    styles.followButton,
                    isFollowed && followStatus !== 'pending' ? styles.unfollowButton : null,
                    followStatus === 'pending' ? styles.pendingButton : null,
                  ]}
                  disabled={followStatus === 'pending' || followLoading}
                  onPress={async () => {
                  console.log("profile?.debug_info?.profile_user_id",userId,isFollowed);
                  
                    if (isFollowed) {
                      // Unfollow API call
                      try {
                        setFollowLoading(true);
                        await axios.post('https://pashuahar.com/follower/unfollow/', { user_id: userId },{headers});
                        setIsFollowed(false);
                        setFollowStatus('');
                      ...
                      console.log("profile?.debug_info?.profile_user_id",profile?.debug_info?.profile_user_id);
                      
                      // Follow API call
                      try {
                        setFollowLoading(true);
                        await axios.post('https://pashuahar.com/follower/follow/', {
                          user_id:  userId || +profile?.debug_info?.profile_user_id
                        },{headers});
                        setIsFollowed(true);
                ...
                >
                  {followLoading ? (
                    <ActivityIndicator color={isFollowed ? '#0095f6' : '#fff'} size="small" />
                  ) : (
                    <Text style={
          ...
          {filteredContent.length === 0 ? (
            <View style={styles.emptyContainer}>
              <Icon name={activeTab === 'reels' ? 'play-circle-outline' : 'image-outline'} size={60} color="#ccc" />
