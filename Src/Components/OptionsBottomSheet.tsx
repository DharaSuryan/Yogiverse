      ...
      <View style={styles.sheet}>
        ...
        {onUnfollow ? (
          <TouchableOpacity style={styles.optionRow} onPress={onUnfollow}>
            {React.createElement(Ionicons, { name: 'person-remove-outline', size: 22, color: '#222', style: { marginRight: 12 } })}
            <Text style={styles.optionText}>Unfollow</Text>
          </TouchableOpacity>
        ) : onFollow ? (
          <TouchableOpacity style={styles.optionRow} onPress={onFollow}>
            {React.createElement(Ionicons, { name: 'person-add-outline', size: 22, color: '#222', style: { marginRight: 12 } })}
            <Text style={styles.optionText}>Follow</Text>
          </TouchableOpacity>
        ) : null}
        <TouchableOpacity style={styles.optionRow} onPress={onAbout}>
          {React.createElement(Ionicons, { name: 'person-circle-outline', size: 22, color: '#222', style: { marginRight: 12 } })}
          <Text style={styles.optionText}>About this account</Text>
        </TouchableOpacity>
        {/* Report button */}
        <TouchableOpacity style={[styles.optionRow, { marginTop: 8 }]} onPress={onReport}>
          {React.createElement(Ionicons, { name: 'alert-circle-outline', size: 22, color: '#ed4956', style: { marginRight: 12 } })}
